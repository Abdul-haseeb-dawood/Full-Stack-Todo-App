...
const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
  const { toggleTaskCompletion, openModal } = useTaskContext();
  const { t } = useLanguage();
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className="flex space-x-2">
          ...
          {/* Priority indicator */}
          <span className={`inline-block w-3 h-3 rounded-full ${getPriorityColor(task.priority)}`}
                title={`${t('task.priority')} ${t(`task.priority.${task.priority}`)}`}>
          </span>
        </div>
      </div>
